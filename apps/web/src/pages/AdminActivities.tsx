...
export default function AdminActivities() {
  ...
  return <>
    ...
    {isLoading ? <Loading /> : error ? <ErrorState error={error} /> : !data?.items.length ? <Empty title="No activities found" message={queue ? 'No activities currently require review for these filters.' : 'No activities match these filters.'} /> : <>
      <div className="table-wrap"><table><thead><tr><th>Activity ID</th><th>ALC</th><th>Type / Partner</th><th>Activity date</th><th>Submitted</th><th>Evidence</th><th>Status</th><th>Actions</th></tr></thead><tbody>{data.items.map(({ activity: a, alc }) => <tr key={a.id}><td><Link to={`/admin/activities/${a.id}`} className="font-semibold text-navy hover:text-teal">{a.activity_number}</Link></td><td><b>{alc.alc_code}</b><p className="text-xs text-slate-500">{alc.alc_name}</p></td><td>{a.activity_type}<p className="text-xs text-slate-500">{a.partner?.partner_name ?? 'No partner'}</p></td><td>{formatDate(a.activity_date)}</td><td>{formatDate(a.submitted_at)}</td><td>{a.evidence.length}</td><td><Badge status={a.status} /></td><td><RowActions base="/admin/activities" id={a.id} status={a.status} /></td></tr>)}</tbody></table></div>
