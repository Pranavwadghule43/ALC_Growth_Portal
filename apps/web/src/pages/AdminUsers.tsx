...
import type { Alc, Page, User } from '../types'
import { Badge, ErrorState, Loading, PageHeader } from '../components/ui'

const empty = { username: '', email: '', role: 'ALC', alc_id: '', password: '', must_change_password: true }
...
export default function AdminUsers() {
  ...
  async function save(event: React.FormEvent) {
    ...
    try {
      await api.post('/admin/users', { ...form, email: form.email || null, alc_id: form.role === 'ALC' ? form.alc_id : null })
  ...
  return <>
    ...
    {open && <form onSubmit={save} className="panel mb-5 grid gap-4 p-5 md:grid-cols-2">
      ...
      <div><label>Role</label><select className="mt-1" value={form.role} onChange={e => setForm({ ...form, role: e.target.value })}><option>ALC</option><option>ADMIN</option></select></div>
