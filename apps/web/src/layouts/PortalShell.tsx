...
export default function PortalShell({ user }: { user: User }) {
  ...
  const nav = isSbu ? sbuNav : alcNav
  const primary = isSbu ? (user.sbu?.name ?? user.username) : (user.alc?.alc_name ?? user.username)
  const secondary = isSbu ? (user.sbu?.code ?? 'SBU') : (user.alc?.alc_code ?? 'ALC')
  ...
  return <div className="min-h-screen bg-canvas">
    ...
    <aside className={`fixed inset-y-0 left-0 z-50 flex w-64 flex-col bg-navy text-white transition-transform lg:translate-x-0 ${open ? 'translate-x-0' : '-translate-x-full'}`}>
      <div className="flex h-20 items-center justify-between border-b border-white/10 px-5"><div><p className="text-lg font-bold">ALC Growth Portal</p><p className="text-xs text-slate-300">{isSbu ? 'SBU workspace' : 'Growth & collaboration'}</p></div><button onClick={() => setOpen(false)} className="lg:hidden"><X/></button></div>
      <nav className="flex-1 space-y-1 overflow-y-auto p-3">{nav.map(([label, to, Icon]) => <NavLink key={to} to={to} end={to === '/portal'} onClick={() => setOpen(false)} className={({ isActive }) => `flex items-center gap-3 rounded-md px-3 py-2.5 text-sm font-medium ${isActive ? 'bg-white text-navy' : 'text-slate-200 hover:bg-white/10'}`}><Icon className="h-4 w-4"/>{label}</NavLink>)}</nav>
