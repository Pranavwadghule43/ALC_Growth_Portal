...
export default function PortalShell({ user }: { user: User }) {
  ...
  return <div className="min-h-screen bg-canvas">
    ...
    <aside className={`fixed inset-y-0 left-0 z-50 flex w-64 flex-col bg-navy text-white transition-transform lg:translate-x-0 ${open ? 'translate-x-0' : '-translate-x-full'}`}>
      ...
      <nav className="flex-1 space-y-1 overflow-y-auto p-3">{nav.map(([label, to, Icon]) => <NavLink key={to} to={to} onClick={() => setOpen(false)} className={`flex items-center gap-3 rounded-md px-3 py-2.5 text-sm font-medium ${isNavActive(to, pathname) ? 'bg-white text-navy' : 'text-slate-200 hover:bg-white/10'}`}><Icon className="h-4 w-4"/>{label}</NavLink>)}</nav>
