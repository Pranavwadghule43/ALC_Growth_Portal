...
// (so "Add Activity" does not also light up "My Activities").
function activeLink(pathname: string, links: readonly string[], home: string) {
  return links
...
export default function AdminShell({ user }: { user: User }) {
  ...
  const current = activeLink(pathname, nav.map(([, to]) => to), '/admin')
  ...
  return <div className="min-h-screen bg-canvas">
    ...
    <aside className={`fixed inset-y-0 left-0 z-50 flex w-64 flex-col bg-navy text-white transition-transform lg:translate-x-0 ${open ? 'translate-x-0' : '-translate-x-full'}`}>
      ...
      <nav className="flex-1 space-y-1 overflow-y-auto p-3">{locked && <p className="rounded-md bg-amber-400/15 px-3 py-2 text-xs text-amber-100">Change your password on this page to unlock the admin console.</p>}{nav.map(([label, to, Icon]) => <Link key={to} to={to} aria-current={to === current ? 'page' : undefined} onClick={() => setOpen(false)} className={`flex items-center gap-3 rounded-md px-3 py-2.5 text-sm font-medium ${to === current ? 'bg-white text-navy' : 'text-slate-200 hover:bg-white/10'}`}><Icon className="h-4 w-4 shrink-0"/><span className="truncate">{label}</span></Link>)}</nav>
